...
@dataclass
class OppoCommentData(CommentData):
    """OPPO评论数据（扩展版）"""
...
class OppoAppStoreAdapter(BaseAppStoreAdapter):
    ...
    APP_PACKAGE = "com.heytap.market"
    MARKET_SCHEME = "market://details?id={package}"

    # 评论元素定位符
    APP_DETAILS_BTN_ID = "com.heytap.market:id/show_more_text"
    COMMENT_MORE_BTN_ID = "com.heytap.market:id/tv_more_text"
    SORT_BTN_ID = "com.heytap.market:id/tv_sort"  # 排序按钮（最热/最新）

    # 评论数据定位符
    USER_ID_ID = "com.heytap.market:id/tv_item_username"
    COMMENT_CONTENT_ID = "com.heytap.market:id/expand_tv"
    COMMENT_TIME_ID = "com.heytap.market:id/tv_item_time"
    COMMENT_LIKE_ID = "com.heytap.market:id/tv_item_praise"
    ...
    def open_details_by_package(self, package_name: str) -> None:
        ...
        # 点击"应用详情"
        details_btn = self.d(resourceId=self.APP_DETAILS_BTN_ID)
        if details_btn.exists:
            details_btn.click()
            time.sleep(2)

        # 点击"更多"进入评论页
        more_btn = self.d(resourceId=self.COMMENT_MORE_BTN_ID)
        if more_btn.exists:
            more_btn.click()
            time.sleep(2)

            # 切换到"最新"排序（默认是"最热"）
            sort_btn = self.d(resourceId=self.SORT_BTN_ID)
            if sort_btn.exists:
                sort_text = sort_btn.info.get('text', '')
                if sort_text == '最热':
                    sort_btn.click()
                    time.sleep(1)
                    # 点击弹出的"最新"选项
    ...
    def extract_comments(self, config: OppoScrapeConfig) -> List[OppoCommentData]:
        """提取当前页面的评论数据"""
        comments = []
        ...
        for i in range(min_count):
            try:
                ...
                comments.append(OppoCommentData(
                    content=content,
                    user_id=user_id,
                    rating=None,
                    comment_date=None,
                    time_str=time_str,
                    time_type="完整日期" if time_str else "",
                    days_ago=days_ago
                ))
    ...
    def scrape_game_comments(
        ...
        game_name: str,
        package_name: str,
        config: Optional[OppoScrapeConfig] = None
    ) -> List[OppoCommentData]:
        """
        完整的评论采集流程
        ...
        # 采集评论
        all_comments: List[OppoCommentData] = []
        seen_contents = set()
        scroll_count = 0
        ...
        while scroll_count < config.max_scrolls:
            ...
            if scroll_count % config.progress_interval == 0:
                ...
                # 默认输出
                print(f"  滚动 {scroll_count}: 已采集 {len(all_comments)} 条 | 最旧: {max_days}天前", flush=True)
    ...
    def scrape_to_legacy_format(
        ...
        """采集并转换为旧格式（向后兼容）"""
        config = OppoScrapeConfig(max_count=max_count)
        oppo_comments = self.scrape_game_comments(game_name, package_name, config)

        # 转换为旧格式
        legacy_comments = []
        for oc in oppo_comments:
            legacy_comments.append(CommentData(
                content=oc.content,
                user_id=oc.user_id,
                rating=oc.rating,
                comment_date=oc.comment_date
            ))
